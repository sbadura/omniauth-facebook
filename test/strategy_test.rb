...
class CallbackUrlTest < StrategyTestCase
  test "returns the default callback url (omitting querystring)" do
    ...
    assert_equal "#{url_base}/auth/instagram/callback", strategy.callback_url
  end
...
module SignedRequestTests
  ...
  class CookieAndParamNotPresentTest < TestCase
    ...
    test 'throws an error on calling build_access_token' do
      assert_raises(OmniAuth::Strategies::Instagram::NoAuthorizationCodeError) { strategy.send(:with_authorization_code!) {} }
    end
  end
  ...
  class CookiePresentTest < TestCase
    ...
    test 'throws an error if the algorithm is unknown' do
      setup('UNKNOWN-ALGO')
      assert_equal "unknown algorithm: UNKNOWN-ALGO", assert_raises(OmniAuth::Instagram::SignedRequest::UnknownSignatureAlgorithmError) { strategy.send(:signed_request_from_cookie) }.message
    end
  end
  ...
  class MissingCodeInParamsRequestTest < TestCase
    ...
    test 'calls fail! when a code is not included in the params' do
      strategy.expects(:fail!).times(1).with(:no_authorization_code, kind_of(OmniAuth::Strategies::Instagram::NoAuthorizationCodeError))
  ...
  class UnknownAlgorithmInCookieRequestTest < TestCase
    ...
    test 'calls fail! when an algorithm is unknown' do
      strategy.expects(:fail!).times(1).with(:unknown_signature_algorithm, kind_of(OmniAuth::Instagram::SignedRequest::UnknownSignatureAlgorithmError))
      strategy.callback_phase
    end
  end
end
